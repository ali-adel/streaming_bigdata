...
def insert_into_phpmyadmin(row):
    ...
    sql_query = f"INSERT INTO user (temp_avg,max_wind,presure_avg) VALUES ({column1_value},{column2_value},{column3_value})"
...
#########################################################################################
df = df.groupBy("data.Precip_Type").agg(
    round(mean("data.Temperature"), 2).alias("Temperature"),
    round(max("data.Wind_Speed"), 2).alias("Wind_Speed"),
    round(mean("data.Pressure"), 2).alias("Pressure")
)
...
#df = df.select("Temperature","Wind_Speed","Pressure")
